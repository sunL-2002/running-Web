  ...
  <div class="dashboard-container">
    <div class="container">
      ...
      <el-table v-if="tableData.length"
        ...
        <el-table-column prop="groupDescription"
                         label="跑团描述" 
                         width="700px"
                         max-height="100px">
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.groupDescription" placement="top">
              <p class="showOverTooltip">{{scope.row.groupDescription}}</p>
            </el-tooltip>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            roundx
            @click="addDishtype(scope.row.groupId,scope.row.groupName)">查看</el-button>
          </template>
        </el-table-column>
...
export default class extends Vue {
  ...
  private async init(isSearch?) {
    ...
    await getDishPage({
      ...
      groupName: this.input || undefined,
    })
      .then(res => {
        if (res.data.code === 1) {
  ...
  // 添加
  private addDishtype(st: string,name: string) {
    if (st === 'add') {
      this.$router.push({ path: '/dish/add' })
    } else {
      this.$router.push({ path: '/dish/add', query: { id: st ,groupName:name} })
    }
  }
  ...
  private deleteHandle(type: string, groupId: any) {
    ...
    this.$confirm('确认删除该跑团, 是否继续?', '确定删除', {
  ...
  private statusHandle(row: any) {
    ...
    this.$confirm('确认更改该跑团状态?', '提示', {
      ...
      dishStatusByStatus(this.dishState)
        .then(res => {
          if (res && res.data && res.data.code === 1) {
            this.$message.success('跑团状态已经更改成功！')
            this.init()
          } else {
            this.$message.error(res.data.msg)
          }
